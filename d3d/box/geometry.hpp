...
struct Point2
{
    ...
    CUDA_CALLABLE_MEMBER Point2 operator+(const Point2& v2) { return Point2(x + v2.x, y + v2.y); }
    CUDA_CALLABLE_MEMBER Point2 operator-(const Point2& v2) { return Point2(x - v2.x, y - v2.y); }
};
...
struct Line2
{
    ...
    CUDA_CALLABLE_MEMBER float dist(const Point2& p) const { return (a*p.x + b*p.y + c) / sqrt(a*a + b*b); }

    // Calculate intersection point of two lines
    CUDA_CALLABLE_MEMBER Point2 intersect(const Line2& l) const
    {
        float w = a*l.b - l.a*b;
