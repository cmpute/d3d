...
struct _BoxUtilCpu<scalar_t, d3d::Box2<scalar_t>>
{
    static d3d::Box2<scalar_t> make_box(const _CpuAccessor(1) data)
    {
        return d3d::poly2_from_xywhr(data[0], data[1], data[2], data[3], data[4]);
    }
    static void make_box_grad(const _CpuAccessor(1) data, const d3d::Box2<scalar_t> &grad, _CpuAccessor(1) grad_data)
    {
