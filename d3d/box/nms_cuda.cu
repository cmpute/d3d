...
__global__ void rbox_2d_nms_kernel(
    const _CudaAccessor(2) boxes,
    const _CudaAccessorT(int64_t, 1) order,
    const scalar_t threshold,
    _CudaAccessorT(int64_t, 2) mask
) {
...
__global__ void nms_collect(
    const _CudaAccessorT(int64_t, 1) order,
    const _CudaAccessorT(int64_t, 2) mask,
    _CudaAccessorT(int64_t, 1) remv,
    _CudaAccessorT(bool, 1) suppressed // need to be filled by false
) {
...
Tensor nms2d_cuda_template(
  const Tensor boxes, const Tensor order, const float threshold
    ...
    switch(KernelType)
    {
        default:
        case 0:
            AT_DISPATCH_FLOATING_TYPES(boxes.scalar_type(), "box_2d_nms_kernel", ([&] {
                box_2d_nms_kernel<<<blocks, threads>>>(
                    boxes._cuda_accessor(2),
                    order._cuda_accessor_t(int64_t, 1),
                    (scalar_t) threshold,
                    mask._cuda_accessor_t(int64_t, 2));
            }));
            break;
        case 1:
            AT_DISPATCH_FLOATING_TYPES(boxes.scalar_type(), "rbox_2d_nms_kernel", ([&] {
    ...
    nms_collect<<<1, 1>>>(
        order._cuda_accessor_t(int64_t, 1),
        mask._cuda_accessor_t(int64_t, 2),
        remv._cuda_accessor_t(int64_t, 1),
