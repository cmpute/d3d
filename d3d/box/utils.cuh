...
struct _BoxUtilCuda<scalar_t, d3d::Box2<scalar_t>>
{
    static CUDA_CALLABLE_MEMBER d3d::Box2<scalar_t> make_box(const _CudaSubAccessor(1) data)
    {
        return d3d::poly2_from_xywhr(data[0], data[1], data[2], data[3], data[4]);
    }
    static CUDA_CALLABLE_MEMBER d3d::Box2<scalar_t> make_box(const __restrict__ scalar_t* data)
    {
        return d3d::poly2_from_xywhr(data[0], data[1], data[2], data[3], data[4]);
    }
    static CUDA_CALLABLE_MEMBER void make_box_grad(const _CudaSubAccessor(1) data,
        const d3d::Box2<scalar_t> &grad, _CudaSubAccessor(1) grad_data)
    {
...
struct _BoxUtilCuda<scalar_t, d3d::AABox2<scalar_t>>
{
    ...
    {
        d3d::Box2<scalar_t> poly, grad_poly;
